...
public class CustomerService implements UserDetailsService {
	...
	private static final Logger log = LoggerFactory.getLogger(CustomerService.class); // Logger instance with 'log' as
																						// variable name
	...
	/**
	 * Registers and saves the customer details after validating the required
	 * fields. This method checks for the presence of essential fields (first name,
	 * last name, email, and password) in the provided CustomerDTO. If any of the
	 * fields are missing, it throws a BAD_REQUEST error.
	 * 
	 * If all fields are valid, it hashes the password using BCrypt and saves the
	 * customer in the repository. The method also handles cases where the email
	 * already exists (DataIntegrityViolationException) or other unexpected errors
	 * that may occur during the registration process.
	 *
	 * @param customerDTO The customer details to be registered, including first
	 *                    name, last name, email, and password.
	 * @return The saved Customer entity after registration.
	 * @throws ResponseStatusException If required fields are missing or if an error
	...
	public Customer register(CustomerDTO customerDTO) {
		try {
			...
			String encodedPassword = passwordEncoder.encode(customerDTO.getPassword()); // Use the injected password
																						// encoder
	...
	/**
	 * loadUserByUsername is used to extract the user details from the database
	 * using the provided email. This method is typically called during the
	 * authentication process when a user tries to log in with their email. It loads
	 * the user's details from the database, and returns a UserDetails object with
	 * the user's email and password which is used by Spring Security for
	 * authentication and authorization.
	 *
	 * @param email The email of the user to be authenticated.
	 * @return UserDetails The user details, including the email and hashed
	 *         password.
	...
	/**
	 * This method handles the logout functionality by invalidating the provided JWT
	 * token. It extracts the username from the token, adds the token to the
	 * blacklist (to prevent reuse), and logs the user out.
	 *
	...
	public void logout(String token) {
		try {
			...
			/**
			 * Create a BlacklistedToken object with the token and username. This object
			 * will store the token in the blacklist to prevent future use of this token
			 */
			BlacklistedToken blacklistedToken = new BlacklistedToken(token, username);

			/**
			 * Save the blacklisted token to the repository. This ensures that the token
			 * will no longer be valid in future authentication requests
			 */
			blacklistedTokenRepository.save(blacklistedToken);
			log.info("User logged out successfully : {}", username);
		} catch (Exception e) {
			log.error("An error occurred while logging out user with token: {}", token, e);
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "An error occurred while logging out",
					e);
		}
	}
