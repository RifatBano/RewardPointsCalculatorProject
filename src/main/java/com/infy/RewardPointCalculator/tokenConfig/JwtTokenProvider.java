...
/**
 * This class provides functionality for generating, validating, and resolving
 * JWT tokens in the application. It also ensures that tokens are checked for
 * validity and expiration, and supports the management of blacklisted tokens.
 * The class is integrated with Spring Security to work as an authentication
 * mechanism based on JWT tokens.
 */
@Component
@Slf4j
public class JwtTokenProvider {
	...
	/**
	 * This method generates a JWT token for a given authenticated user. The token
	 * includes the username, issued time, and expiration time (1 hour from now).
	 * 
	...
	/**
	 * This method extracts the token from the Authorization header of the HTTP
	 * request. It checks for the "Bearer" prefix and ensures the token is not
	 * blacklisted.
	 * 
	 * @param request The HTTP request to extract the token from.
	 * @return The JWT token if present and valid, or null if not found or
	 *         blacklisted.
	 */
	...
	/**
	 * Validates the JWT token by parsing it using the signing key and checking for
	 * common exceptions like malformed, expired, or unsupported tokens.
	 * 
	...
	/**
	 * Determines if the provided token has expired by comparing its expiration date
	 * to the current date.
	 * 
	 * @param token The JWT token to check.
	...
	/**
	 * Checks whether the token is blacklisted by querying the BlacklistedToken
	 * repository.
	 * 
	 * @param token The JWT token to check.
