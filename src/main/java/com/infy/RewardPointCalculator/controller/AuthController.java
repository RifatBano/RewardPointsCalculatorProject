...
public class AuthController {
	 ...
	 * 
	 * This endpoint handles the user authentication by verifying the provided credentials
	 * (email and password). If the authentication is successful, it generates a JWT token
	 * and returns it to the user for subsequent requests. If authentication fails, 
	 * it returns an appropriate error message.
	 * 
	 * @param authenticationRequest contains the user's email and password for authentication
	 * @return ResponseEntity with JWT token if authentication is successful, or an error message if not
	 */	
	@PostMapping("/login")
	 ...
	 * 
	 * This endpoint handles the user logout process by invalidating the provided JWT token. 
	 * It extracts the token from the Authorization header, blacklists it using the `CustomerService`, 
	 * and logs the user out. If the logout process is successful, it returns a success message.
	 * 
	 * @param token the JWT token passed in the Authorization header, which is used to identify the user
	 * @return ResponseEntity indicating the outcome of the logout process (success or failure)
	 */	
	@PostMapping("/logout")
