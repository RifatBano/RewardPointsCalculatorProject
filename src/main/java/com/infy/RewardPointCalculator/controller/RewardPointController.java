...
/**
 * RewardPointController handles get apis to get rewardpoint details month and
 * yearwise or the aggregated points for the authenticated user
 * 
 * @author rifat.bano
 */
@RestController
...
public class RewardPointController {
	 ...
	 * 
	 * This endpoint retrieves the reward points for a customer based on their
	 * logged-in username (email), and the specified month and year. If the user is
	 * not logged in or an error occurs, an appropriate response is returned.
	 * 
	 ...
	 * @return ResponseEntity with the reward points data or an error response if
	 *         any issues occur
	 */
	...
	public ResponseEntity<RewardPoints> getRewardPoints(@PathVariable Integer month, @PathVariable Integer year) {
		...
		try {
			...
			log.error("Error occurred while fetching reward points for user with email: {} for {}/{}", loggedInUsername,
					month, year, e);
	 ...
	 * 
	 * @return ResponseEntity with a list of all reward points or an error response
	 *         if any issues occur
	 */
	@GetMapping("/all")
