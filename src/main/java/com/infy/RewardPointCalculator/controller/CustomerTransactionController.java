...
/**
 * CustomerTransactionController handles all customer transactions :get, add, edit and delete
...
public class CustomerTransactionController {
     ...
     * 
     * This endpoint fetches all transactions for the currently authenticated user by retrieving their transactions
     * from the transaction service and returns them in the response body.
     * 
     * @return ResponseEntity with a list of customer transactions or an error message if an issue occurs
     */
    @GetMapping
     ...
     * 
     * This endpoint allows the logged-in user to add a new transaction by submitting transaction details in the request body.
     * The transaction is added via the transaction service.
     * 
     * @param transactionDTO Data transfer object containing the transaction details
     * @return ResponseEntity with the newly created transaction or an error message if the operation fails
     */    
    @PostMapping
    public ResponseEntity<CustomerTransaction> addTransaction(@RequestBody @Valid CustomerTransactionDTO transactionDTO) {
        String loggedInUsername = UserUtil.getLoggedInUsername();  // Get logged-in username
     ...
     * 
     * This endpoint allows the logged-in user to edit an existing transaction by providing the transaction ID
     * and updated details in the request body.
     * 
     * @param transactionId The ID of the transaction to be updated
     * @param transactionDTO Data transfer object containing the updated transaction details
     * @return ResponseEntity with the updated transaction or an error message if the operation fails
     */    
    ...
    public ResponseEntity<CustomerTransaction> editTransaction(@PathVariable Long transactionId,
        ...
        try {
            ...
            CustomerTransaction updatedTransaction = transactionService.editTransaction(customer.getId(), transactionId, transactionDTO);
            log.info("Transaction with ID: {} updated for customer with email: {}", transactionId, loggedInUsername);
            return ResponseEntity.ok(updatedTransaction);

        } catch (Exception e) {
            log.error("Error occurred while editing transaction with ID: {} for user with email: {}", transactionId, loggedInUsername, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
    }
     ...
     * 
     * This endpoint allows the logged-in user to delete a transaction by providing the transaction ID.
     * The transaction is removed via the transaction service.
     * 
     * @param transactionId The ID of the transaction to be deleted
     * @return ResponseEntity with a no-content response or an error message if the operation fails
     */
    ...
    public ResponseEntity<Void> deleteTransaction(@PathVariable Long transactionId) {
        ...
        try {
            ...
            log.error("Error occurred while deleting transaction with ID: {} for user with email: {}", transactionId, loggedInUsername, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
    }
}
