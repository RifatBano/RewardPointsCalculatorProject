 ...
 * <p>
 * This class contains tests for various methods in the {@link CustomerTransactionService},
 * such as adding, editing, deleting, and fetching transactions. It also covers scenarios like 
 * when customers or transactions are not found.
 * </p>
 */
...
public class CustomerTransactionServiceTest {
    ...
    /**
     * This method sets up the test data before each test case is run.
     * Initializes a mock customer, transaction, and DTO object for the tests.
     */
    @BeforeEach
    public void setUp() {
    ...
    /**
     * Tests the {@link CustomerTransactionService#addTransaction(Long, CustomerTransactionDTO)} method.
     * Verifies that a transaction is successfully added for an existing customer.
     */
    @Test
     ...
     * Verifies that an error response is returned when trying to add a transaction for a non-existent customer.
     */
    @Test
    public void testAddTransaction_CustomerNotFound() {
        when(customerRepository.findById(1L)).thenReturn(Optional.empty());

        ResponseStatusException exception = assertThrows(ResponseStatusException.class, () -> customerTransactionService.addTransaction(1L, transactionDTO));
    ...
    /**
     * Tests the {@link CustomerTransactionService#editTransaction(Long, Long, CustomerTransactionDTO)} method.
     * Verifies that a transaction is successfully edited when both the customer and transaction exist.
     */
    @Test
     ...
     * Verifies that an error response is returned when trying to edit a non-existent transaction.
     */
    @Test
    public void testEditTransaction_NotFound() {
        when(transactionRepository.findByCustomerIdAndId(1L, 1L)).thenReturn(Optional.empty());

        ResponseStatusException exception = assertThrows(ResponseStatusException.class, () -> customerTransactionService.editTransaction(1L, 1L, transactionDTO));
    ...
    /**
     * Tests the {@link CustomerTransactionService#deleteTransaction(Long, Long)} method.
     * Verifies that a transaction is successfully deleted for an existing customer and transaction.
     */
    @Test
     ...
     * Tests the {@link CustomerTransactionService#deleteTransaction(Long, Long)} method
     * when the transaction is not found.
     * Verifies that an error response is returned when trying to delete a non-existent transaction.
     */
    @Test
    public void testDeleteTransaction_NotFound() {
        ...
        ResponseStatusException exception = assertThrows(ResponseStatusException.class, () -> customerTransactionService.deleteTransaction(1L, 1L));
     ...
     * Tests the {@link CustomerTransactionService#getCustomerById(Long)} method
     * when the customer is not found.
     * Verifies that an exception is thrown if the customer is not found.
     */
    @Test
    public void testGetCustomerById_CustomerNotFound() {
        when(customerRepository.findById(1L)).thenReturn(Optional.empty());

        RuntimeException exception = assertThrows(RuntimeException.class, () -> customerTransactionService.getCustomerById(1L));
